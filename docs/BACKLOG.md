...
**B16 is done**, so the table now jumps B15 to B17 and its row is in *Resolved* at the bottom
rather than deleted. Same rule: the gap is a moved row. *Resolved* is a third disposition
alongside *the legacy image builder* and *Closed by the copy* — not-ours, closed-without-doing,
and done — because a backlog that only ever loses rows cannot be asked what it shipped.
...
Done, with the commit that did it and the measurement that says so. A row moves here rather than
being deleted, for the reason the core section states: an ID is allocated once and a gap is not a
deletion.

| # | Item | Resolved |
|---|---|---|
| B16 | Correct `modules/ledger/docs/commands.md` to the five-name export list, retire its row from `modules/ledger/tests/fixtures/copied-blobs.psd1:37`, and drop `modules/ledger/tests/ledger.Tests.ps1:348` from `10` to `9` — one pull request, one receipt, the shape **B10** already describes | `88d507b`, receipt **seq 16** `retire-ledger-commands-pin`. All three files in one commit, on the precedent of `d182cbe`. Measured: `Files` `10` rows to `9`, `ledger.Tests.ps1:348` `10` to `9`, `commands.md:3-8` rewritten against `modules/ledger/ledger.psd1:9`. Suite `206` to `205 passed / 0 failed`, the one-test delta in `modules/ledger/tests/ledger.Tests.ps1` (`72` to `71`) and nowhere else. **D002** held — the row was retired, not re-pinned, and the reasoning is written into the fixture. `modules/ledger/docs/theory-of-operation.md` stays pinned and untouched |
