...
**B17 and B18 are reserved, not retired.** The core table jumps from B16 to B19 because the two
numbers between them are held for work named elsewhere and not yet written down here. A gap in
this column is not a deleted row: IDs here are allocated once and never reused, on the same
