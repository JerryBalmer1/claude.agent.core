...
function Get-RepoConfigAtRef {
    ...
    #
    # Found in the legacy repo, which ported this file and hit the case on its very
    # first pull request. Reading the config from the base branch has a bootstrap consequence:
    # the pull request that INTRODUCES config/repo.json is, by definition, opened against a base
    # that does not have it. The API answers 404, and with $PSNativeCommandUseErrorActionPreference
    # the bare call threw a NativeCommandExitException and failed the whole automerge run.
    #
    # That contradicts this script's own contract, stated at the top of Invoke-AutoMerge.ps1:
    # "Exits 0 in every 'not yet' case - a PR that is not ready is not an error, and a red
    # automerge run on every push would be noise that hides a real failure."
    #
    # LATENT HERE, LIVE THERE. This repository's base already carries config/repo.json, so the
    # 404 path has never been taken. It is fixed anyway: a bug that has not fired yet is still
    # a bug, and the next repository built from this pattern hits it on day one.
    #
    # $null rather than a default config is the safe direction - no config means no
    # authorisation, so a human merges. The other choice is an automerge that invents its own
    # permission.
    # NOT $LASTEXITCODE. The first version of this fix read it, and three passing tests went red
    # immediately: $LASTEXITCODE is only ever set by a NATIVE command, and tests/AutoMerge.Tests.ps1
    # shadows `gh` with a FUNCTION. The variable kept whatever value it already had - frequently
    # $null - so the guard rejected perfectly good configs and the gate stood down on everything.
    # In production it would have been worse than useless: silently correct until some earlier
    # native call left a non-zero behind.
    #
    # The caller sets $PSNativeCommandUseErrorActionPreference = $true and those preferences are
    # dynamically scoped, so a real `gh` failure throws in here and the catch takes it. A shim
    # that returns normally never throws. Both paths land where they should.
    #
    # The empty-output check is the belt to that brace: if a caller has native errors turned
    # off, a failed `gh` returns nothing instead of throwing, and "nothing" must not be decoded
    # as a config.
    $encoded = $null
    try {
