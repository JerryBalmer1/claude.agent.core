...
#
# COPIED, NOT VENDORED.
#   origin repo   : claude.pwsh.image.builder
#   origin file   : scripts/forensic.ps1
#   origin commit : 06e738dda71f16a28977a1baf17eb7e70847b0f1
#   origin sha256 : ad16a85b355e00908a69f6c688937612e2aa51ff8d35ce61c8a58cfdc6404852
#
# Copied verbatim on 2026-09-21. There is no vendor/ directory here and this file is not a
# submodule - substrate owns its copy outright. If the image builder's version moves, this one
# does not follow by itself. Diff the two against the origin commit above before assuming they
# still agree.
#
# NO LONGER VERBATIM. Two ValidateSets are narrower here than in the file this was copied from:
# `actor` is `claude` and nothing else, and `kind` drops the `arena-utterance` value the copy
# carried. Both are subtractive and neither is a change to the record schema: the eight keys are
# untouched, `kind` is still a string, and every `self` already written hashes the same bytes it
# did before. A roster is a claim about who may write, and this repository has one writer.
#
<#
.SYNOPSIS
...
.DESCRIPTION
    ...
    Schema forensic-v1, eight keys, in this order, frozen from birth:
        ...
        actor     claude   -- operator-asserted, see SHARP EDGES
...
param(
    ...
    [ValidateSet('claude')] [string]$Actor,
    [Parameter(ParameterSetName = 'Append', Mandatory)]
